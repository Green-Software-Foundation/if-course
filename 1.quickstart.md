...
2. Download [this sample manifest file](https://raw.githubusercontent.com/Green-Software-Foundation/if-course/refs/heads/main/src/quickstart-imp.yaml), which contains everything needed to compute the environmental impacts of a website (the GSF website), and save it as `quickstart-imp.yml` inside `my-first-imp`
...
> [!TIP]
> To save a few characters, you can type `if-run -m quickstart-imp -o quickstart-output`

Now, if you open up the file `quickstart-output` and go to the end of the file, you will see a value for carbon and software carbon intensity (SCI) that is the same as below.
...
This is the slightly more complicated option, but it is favoured because it keeps the files local to your computer.

Navigate to the folder where `quickstart-output.yaml` lives and start a simple HTTP server:
...
1. Upload `output-file.yml` to a GitHub repository
2. Get the raw file URL (e.g., `https://raw.githubusercontent.com/yourusername/yourrepo/main/output-file.yml`)
...
The aggregated carbon figure has now changed.
