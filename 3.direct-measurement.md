...
- [One-Step Pipeline](#one-step-pipeline)
  ...
  - [IDOMI](#idomi)
  - [Impacts](#impacts)
  - [Dependencies \& Observations](#dependencies--observations)
...
Now we have an understanding of the basics of IF and IMPs, let's walk you through a simple IMP that converts energy consumption into carbon emissions using the built‑in `Multiply` plugin.
...
First off let's create the boilerplate empty imp file which we will continue to work with for the rest of the module.
...
We start to build up a **impact dependency tree**, this is a useful tool which can help us figure out what data we need and what methodologies we should be applying. 
...
In this simple example we are lucky enough to be using a cloud provider which already measures and exposes the energy consumption of our server in their monitoring environment. So the **observation** we can gather is `energy`. Let's add this to our impact dependency tree like so:

☑️ carbon 
- 🌿 energy

> [!NOTE] 
> An **impact** or an **impact dependency** is something we need to compute, an **observation** is something we will **use** in a computation. Since energy is an observation it also means it's a leaf in our impact dependency tree. I like to visually denote an observation with 🌿 to distinguish between an impact/dependency.

We will need a **methodology** that **induces** `energy` into `carbon`, there is a well known methodology using the carbon intensity of electricity. Let's add `carbon-intensity` as another observation we will need to gather in our impact dependency tree.
...
- <2> The duration of the observation in seconds, here it's 1 hour (3600 seconds).
- <3> The energy in kWh
...
server-1:
  ...
  inputs:
    - timestamp: 2023-08-06T00:00
      duration: 3600
      energy: 0.05
      carbon-intensity: 163  # <<--- here is the carbon intensity!!
```
...
So in order to convert the energy into carbon we want a model that lets us multiply the carbon intensity of electricity by the energy. Luckily there is a built-in plugin called `Multiply` that lets you do this.

> [!NOTE] Finding model plugins
> You can check the [standard library of "builtins"](https://if.greensoftware.foundation/reference/plugins) to find available plugins. The builtins are convenient because they are bundled with IF - there's no need to install any third-party code.
>
> For this calculation, we could use either the `Multiply` plugin (which multiplies multiple input values together) or the `Coefficient` plugin (which multiplies a single input by a fixed coefficient). We'll use `Multiply` here since it's more flexible when working with multiple dynamic inputs.
>
> If you need more specialized functionality, you can search the [Plugin Explorer](https://explorer.if.greensoftware.foundation/) for community-developed plugins, or build your own custom plugin. Impact Framework is flexible and extensible for your use case.

To use the `Multiply` plugin we first need to add it to the context, specifically in the `initialize: plugins:` block like so:
...
- <6> `output-parameter` is the name of the parameter you want the result of the multiplication output to. 

> [!NOTE]
> In more complicated IMPs you might have many instances of the same plugin (e.g. you might use the `Multiply` plugin many times). Each instance we call a model, you can call models whatever you like, but we recommend using very descriptive names, using hyphen separated words.
...
- We observed that for a 1 hour period of time the energy consumed by `server-1` was 0.05 kWh.
- We added an observation to `server-1` by adding an element to the `inputs` array with an energy parameter of `0.05`.
- We configured a `Multiply` model plugin called `carbon-from-energy` which multiplies `energy` by `carbon-intensity` to calculate `carbon` emissions.
- We configured the model plugin to run on the `server-1` component, by adding it to the `pipeline` of the component.

Now it's time to actually compute the carbon, make sure you have saved your file as `single-server.yml` and run the command below:

```bash
if-run -m single-server.yml
```

You will see some logs appear in your console, and then the output IMP file will be printed there too. You will notice that two blocks of YAML have been added to your original IMP data: `execution` and `outputs`, like so:
  ...
  plugins:
    carbon-from-energy:
      ...
      config:
        ...
        output-parameter: carbon
execution:  # <1>
  command: if-run -m single-server.yml
  environment:
    if-version: 1.1.0
    os: macOS
    os-version: 14.7.5
...
- How to configure a model plugin.
...
> 
> There is also the WattTime plugin available from the plugin explorer which pulls live Carbon Intensity data by calling the WattTime API so you don't have to hardcode values in the IMP files.

## Test your learning

1. Try using the `Coefficient` plugin instead of `Multiply` to achieve the same result. (Hint: You'll need to change the config to use a single `input-parameter` and a fixed `coefficient` value.)
2. Try varying the values for `energy` and `carbon-intensity` and observe the changing output.
