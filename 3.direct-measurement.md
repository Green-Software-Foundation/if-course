...
The standard development process for a manifest involves these 4 steps.
...
A good mnemonic to remember this is **IOMI** (Io'Mi').

In the example below, the component we want to measure is that of a **server**. 

In this simple example we are lucky enough to be using a cloud provider which already measures and exposes the energy consumption of our server in their monitiring environment. So the **observation** we can gather is `energy`.

The **impact** we want to measure is `carbon`.

So the **methodology** we are looking for is one that **induces** `energy` into `carbon`, there is a well known methodology using the carbon intensity of electricity.
