...
Where you have a parent component composed of multiple children, `aggregate` adds a new `outputs` array to the parent, so that every component in the tree has an `outputs` array, and it is always the aggregated time series of all of its children.
