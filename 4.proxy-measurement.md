...
- [Proxy Measurements](#proxy-measurements)
  - [IDOMI](#idomi)
  - [Impacts](#impacts)
  - [Dependencies \& Observation](#dependencies--observation)
...
We start with the same impact dependency tree as the last module, like so:
...
You will need to induce `memory utilisation` into `memory-energy` using a methodology. Memory energy is therefore an impact dependency and memory utilisation is an observation. Let's add these to our impact dependency tree like so:
...
Let's add this as an observation to our server like so:
...
They suggest that multiplying memory utilisation in gigabytes by approximately `0.00039 kWh/GBh` is appropriate for a first-order estimate of the energy consumption due to memory. This value is derived from averaging the values proposed by two industry power models.

Now you can apply your coefficient to yield an energy value in kWh.

```mathematica
8.8 (GB) * 0.00039 (kWh/GBh) = 0.003432 kWh
```

Let's represent this as a model in the manifest file. Since the model is a simple coefficient that will be the same for all components in our manifest we will use the `Coefficient` plugin. Add the below to the plugin -> initialize section in the manifest file.
```yaml
...
- <3>: The coefficient we researched in the methodology section above.
...
And let's add it to the pipeline for the component like so:
...
Looking at our impact dependency tree:
...
We now have everything we need to compute carbon, which is simply the `energy` multiplied by the `carbon-intensity` which we implemented in the previous module already like so:
...
```bash
if-run -m server-carbon.yml -o out.yaml
```
  ...
  plugins:
    ...
    sum-energy:
      ...
      config:
        ...
        output-parameter: energy
execution:
  command: if-run -m server-carbon.yml
  environment:
    if-version: 1.1.0
    os: macOS
    os-version: 14.7.5
