...
**Software Carbon Intensity (SCI)** is a crucial metric within the realm of green software, designed to provide a standardised way of understanding the **carbon efficiency of software applications**. 

The SCI a versatile tool applicable across different software domains, use cases, and scales. It describes the methodology of calculating the total carbon emissions and the selection criteria to turn the total into a **rate**. By normalizing into a rate of emissions, the SCI facilitates comparisons between different versions of the same software or even between different applications providing similar functionalities. 
...
The Software Carbon Intensity (SCI) technical specification created through consensus of the member organisations at the Green Software Foundation.
...
The latest **development** version of the specification is visible in GitHub here:
...
> [!IMPORTANT] 
> Until then, it's crucial for implementors to apply the current SCI methodically, transparently, and openly. This approach ensures clarity, promotes shared understanding, and helps build consensus on applying SCI effectively across various software domains.
...
The Impact Framework provides several useful plugins for calculating SCI, inclduing:

- SCI
- SCIEmbodied

Which we will cover in more detail in this modulel
...
> [!NOTE] 
> The `tree` in the IMP file denotes clearly and unambiguously the software boundary of your SCI calcualtion. This clarity prevents misleading calculations and ensures transparent, reproducible results.

### Embodied Carbon

Carbon was emitted when the hardware used to run an application was manufactured and more will be emitted when the hardware is disposed of. This is the “embodied” or “embedded” carbon, with the symbol `M` in the `SCI` equation.
...
> [!IMPORTANT]
> If you do have access or can get access to better more accurate data regarding the embodied carbon of your devices. We strongly recommend that you do and **do not use** the above methodology. The methodology and underlying data sources are woefully incomplete since there is a deep lack of public disclosure here. If you do work for a large organization you might have the ability to request detailed embodied carbon values from your suppliers under a non-disclosure agreement. We strongly recommend you go with that approach if you can and only use the above methodology as a fallback.

The Impact Framework `builtins` has a plugin, `SciEmbodied`, for simplyfying the application of the above methodology. You simply specify your hardware in your input data and it calculates the embodied carbon using the Cloud Carbon Footprint model.
...
The CCF model in order to estimate the embodied carbon of your server needs to know several bits of information sich as the number of vCPUs, amount of memory, how many SSD drives etc...

Youc an add these values either to the defalts or inputs of a component like so:
...
Although you could represent a SCI calcualtion with the use of the `Divide` plugin. A more advaced plugin in the standard library is available called `SCI`, the details are available [here](https://github.com/Green-Software-Foundation/if/tree/main/src/if-run/builtins/sci).
    ...
    child:
      ...
      outputs:
        - timestamp: 2023-07-06T00:00
          ...
          sci: 0.05                     # <4>
```
- <1>: `Sci` plugin defines a model which divides `carbon` by whatever is defined as the `functional-unit` and outputs the result as `sci`
...
- <4>: The Sci plugin computes the SCI score and outputs in the `sci` propert.
...
Once you've defined your boundary, functional unit, and methodology, in the SCI specification the next step is quantification, the act of actually computing the SCI score.

It might be obvsious but in practice in Impact Framework you quantify by simply executing your manifest file like so:
