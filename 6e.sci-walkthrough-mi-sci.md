...
The final step is to divide total `carbon` by our functional unit. We chose `site-visits` as our functional unit back in module 6a — it represents the number of visits to our website during the observation period.
...
- <3>: The `functional-unit` config tells the plugin which parameter to divide `carbon` by. In our case, it's `site-visits`. The plugin expects a parameter called `carbon` to already exist in the observations — which is why we sum operational and embodied carbon in the previous step.
...
This single number tells us that for every visit to our website during this hour, approximately 0.41 grams of CO2 equivalent were emitted — accounting for both the operational energy of running the server and the embodied carbon of the hardware it runs on.
