...
The advantage of the RTC data set is that it already has data keyed by cloud provider and cloud region so you don't have to try to figure out where in the world West UK is, however the data is a yearly average so won't surface useful opportunities for carbon aware computing.

### Implementation

<!-- TODO: Add RTC (Real Time Carbon) CSVLookup plugin implementation here to source carbon-intensity from the GSF dataset by cloud-provider and cloud-region. For now, carbon-intensity is hardcoded as 120 in defaults. -->
