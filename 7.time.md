...
Using the same manifest from module 6e with the updated `inputs` above, we run:
...
In our walkthrough we only have a single component (`server-1`) with neatly aligned timestamps. In real-world manifests you might have **multiple components** — for example, a web server, a database server, and a CDN — each with their own time series that may:

- Start and end at different times
- Have different observation frequencies (e.g. one reports every hour, another every 15 minutes)
- Have gaps where data is missing

Impact Framework provides a built-in **`time-sync`** feature to handle this. It synchronises all time series across all components to a common start time, end time, and interval. The process involves:

- **Upsampling** each time series to a common base resolution
- **Gap filling** missing periods with zero-value observations
- **Aligning** start and end times by padding or trimming
- **Batching** into uniform time bins at the configured interval

You configure `time-sync` in the pipeline, and it's generally recommended to place it **first** so all subsequent plugins receive consistently synchronised data:
...
We won't use `time-sync` in this walkthrough since our single component has clean, aligned data. But it becomes essential when you start modelling multi-component systems — which we'll explore in later modules.
